...
func (c *GraphQLClient) CreateEntitlementSpec(name string, spec string) (*EntitlementSpec, error) {
	requestObj := GraphQLRequest{
		Query: `
mutation($spec: String!, $name: String!) {
  createEntitlementSpec(spec: $spec, name: $name) {
    id 
    spec 
    name 
    createdAt
		...
		Variables: map[string]string{"spec": spec, "name": name},
	}
...
func (c *GraphQLClient) SetEntitlementValue(customerID, specID, key, value, datatype string) (*EntitlementValue, error) {
	requestObj := GraphQLRequest{
		Query: `
mutation($customerId: String!, $specId: String!, $key: String!, $value: String!, $type: String!) {
  createEntitlementValue(customerId: $customerId, specId: $specId, key: $key, value: $value, labels: [{key: "type", value: $type}]) {
    id
    key
    value
    labels {
